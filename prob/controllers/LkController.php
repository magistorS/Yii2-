...
class LkController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $newFileName = md5($model->photoBefore . '.' . $model->photoBefore. time()). '.' . $model->photoBefore;
           // $model->photoBefore->saveAs('@webroot/uploads/' . $newFileName);
            $model->photoBefore = $newFileName;
            $modeсl->idUser = Yii::$app->user->identity->id;
            $model->save();
            return $this->redirect(['/lk']);
        }

        $categories = Category::find()->all();
        $mesto = Category::find()->all();

        $categories = ArrayHelper::map($categories, 'id', 'name');
        $mesto = ArrayHelper::map($mesto, 'id', 'mesto');
